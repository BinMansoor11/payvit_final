  ...
  const List = ({ name, phone, index }) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => listPressed(name, phone)}
      >
        {/* <CheckBox
          style={{ alignSelf: "center" }}
          tintColors={{ true: color.DARK_APP }}
  ...
  return isLoading ? (
    ...
    <SafeAreaView style={[globalStyle.flex1]}>
      ...
      <View style={styles.container}>
        <GoBack top="3%" left="8%" size={20} clr={"#fff"} />
        <Text style={styles.head}>
          {/* {Translator.getString("requestMoneyTfr")} */}Money Transfer
        </Text>
            ...
            contactDetails.map((v, i) => {
              return search != null ? (
                v.name.toLowerCase().includes(search.toLowerCase()) ? (
                  <List key={i} name={v.name} phone={v.phoneNumber} index={i} />
                ) : (
